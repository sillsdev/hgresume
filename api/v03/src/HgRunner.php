...
class HgRunner {
	...
	function makeBundle($baseHash, $bundleFilePath, $asyncRunner) {
		...
		if ($baseHash == "0") {
			...
			// TODO JASON need the multiple --base options in here.
	...
	// TODO JASON This could be made head / branch aware which I suspect would help on the client.  I'm think this would be a good idea. CP 2012-08
	function getRevisions($offset, $quantity) {
		if ($quantity < 1) {
			throw new Exception("quantity parameter much be larger than 0");
		}
		chdir($this->repoPath);
		// TODO JASON Checkout the syntax for --template.  Pretty sure you can put in a {branch} or similar.  See selenic for more info.
