...
public sealed class PushFacts
{
    ...
    {
        _fx.SeedRepo("sampleHgRepo.zip");
        var r = Api.PushBundleChunk("sampleHgRepo", 1000, 2000, B("chunkData"), nameof(PushBundleChunk_InvalidOffset_FailCode));
    ...
    {
        _fx.SeedRepo("sampleHgRepo.zip");
        var r = Api.PushBundleChunk("sampleHgRepo", 1000, 0, Array.Empty<byte>(), nameof(PushBundleChunk_NoData_FailCode));
        Assert.Equal("FAIL", r.Status);
    }
    ...
    {
        _fx.SeedRepo("sampleHgRepo.zip");
        var r = Api.PushBundleChunkRaw("sampleHgRepo", "invalid", 0, B("someData"), nameof(PushBundleChunk_InvalidBundleSize_FailCode));
        Assert.Equal("FAIL", r.Status);
    }
    ...
    {
        _fx.SeedRepo("sampleHgRepo.zip");
        var r = Api.PushBundleChunk("sampleHgRepo", 10, 0, B("someDataLargerThan 10 bytes"), nameof(PushBundleChunk_DataTooLarge_FailCode));
        Assert.Equal("FAIL", r.Status);
    }
    ...
    public void PushBundleChunk_ChunkSent_ReceivedCode()
    {
        ...
        var r = Api.PushBundleChunk("sampleHgRepo", 100, 0, B("someChunkData"), tx);
    ...
    public void PushBundleChunk_AllChunksSent_SuccessCode()
    {
        ...
        Api.FinishPushBundle(tx);
        var r = Protocol.PushEntireBundle(Api, "sampleHgRepo", tx, _fx.Fixture("sample.bundle"));
    ...
    public void PushBundleChunk_AllChunksSentButBadDataChunkSoBundleFails_ResetCode()
    {
        ...
        Api.FinishPushBundle(tx);
        Assert.Equal("RECEIVED", Api.PushBundleChunk("sampleHgRepo", 15, 0, B("12345"), tx).Status);
        Assert.Equal("RECEIVED", Api.PushBundleChunk("sampleHgRepo", 15, 5, B("1234"), tx).Status);
        Assert.Equal("RECEIVED", Api.PushBundleChunk("sampleHgRepo", 15, 9, B("1234"), tx).Status);
        var last = Api.PushBundleChunk("sampleHgRepo", 15, 13, B("12"), tx);
        last = Protocol.SettlePush(Api, "sampleHgRepo", tx, B("123451234123412"), last);
    ...
    public void PushBundleChunk_RequestedOffsetNotEqualToSOW_FailCodeReturnsSOW()
    {
        ...
        Api.FinishPushBundle(tx);
        Api.PushBundleChunk("sampleHgRepo", 15, 0, B("12345"), tx);
        var r = Api.PushBundleChunk("sampleHgRepo", 15, 10, B("12345"), tx);
        Assert.Equal("FAIL", r.Status);
    ...
    public void PushBundleChunk_PushWithOffsetZeroButSOWGreaterThanZero_ReceivedCodeReturnsSOW()
    {
        ...
        Api.FinishPushBundle(tx);
        Api.PushBundleChunk("sampleHgRepo", 15, 0, B("12345"), tx);
        var r = Api.PushBundleChunk("sampleHgRepo", 15, 0, B("12"), tx);
        Assert.Equal("RECEIVED", r.Status);
        Assert.Equal(5, r.HgrInt("sow"));
    }
    ...
    public void PushBundleChunk_InitializedRepoWithZeroChangesets_BundleSuccessfullyApplied()
    {
        ...
        Api.FinishPushBundle(tx);
        var r = Protocol.PushEntireBundle(Api, "emptyHgRepo", tx, _fx.Fixture("sample_entire.bundle"));
        Assert.Equal("SUCCESS", r.Status);
    }
    ...
    public void PushBundleChunk_PushOneChunkThenRepoChanges_PushContinuesSuccessfully()
    {
        ...
        for (int guard = 0; guard < 2000; guard++)
        {
            if (sow >= 50 && !changed)
            {
                _fx.AddAndCommit("sampleHgRepo", "fileToAdd.txt", "sample data to add");
                changed = true;
            }
            r = Api.PushBundleChunk("sampleHgRepo", bundleSize, sow, Protocol.Slice(bundle, sow, 50), tx);
    ...
    public void PushBundleChunk_UnrelatedRepo1_FailCode()
    {
        ...
        var r = Api.PushBundleChunk("sampleHgRepo", bundle.Length, 0, bundle, tx);
        r = Protocol.SettlePush(Api, "sampleHgRepo", tx, bundle, r);
        Assert.Equal("FAIL", r.Status);
    }
