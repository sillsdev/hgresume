...
public sealed class PullFacts
{
    ...
    public void PullBundleChunk_InvalidHash_FailCode()
    {
        ...
        Api.FinishPullBundle(tx);
        var r = Api.PullBundleChunk("sample-hg-repo", new[] { "fakehash" }, 0, 50, tx);
    ...
    public void PullBundleChunk_ValidRequestButNoChanges_NoChangeCode()
    {
        ...
        var r = Api.PullBundleChunk("sample-hg-repo", new[] { hash }, 0, 50, tx);
    ...
    public void PullBundleChunk_OffsetZero_ValidData()
    {
        ...
        Api.FinishPullBundle(tx);
        string hash = _fx.FixtureText("sample.bundle.hash");
        var r = Protocol.PullFirstChunk(Api, "sample-hg-repo2", new[] { hash }, 0, 50, tx);
    ...
    public void PullBundleChunk_OffsetGreaterThanZeroAndNoBundleCreated_ResetResponse()
    {
        ...
        Api.FinishPullBundle(tx);
        string hash = _fx.FixtureText("sample.bundle.hash");
        var r = Api.PullBundleChunk("sample-hg-repo2", new[] { hash }, 50, 50, tx);
    ...
    public void PullBundleChunk_OffsetEqualToBundleSize_SuccessCodeWithZeroChunkSize()
    {
        ...
        Api.FinishPullBundle(tx);
        string hash = _fx.FixtureText("sample.bundle.hash");
        var first = Protocol.PullFirstChunk(Api, "sample-hg-repo2", new[] { hash }, 0, 50, tx);
        ...
        var r = Protocol.PullFirstChunk(Api, "sample-hg-repo2", new[] { hash }, bundleSize, 1000, tx);
        Assert.Equal("SUCCESS", r.Status);
    ...
    public void PullBundleChunk_PullUntilFinished_AssembledBundleIsValid()
    {
        ...
        Api.FinishPullBundle(tx);
        string hash = _fx.FixtureText("sample.bundle.hash");
        var (assembled, _) = Protocol.PullEntireBundle(Api, "sample-hg-repo2", new[] { hash }, tx);
    ...
    public void PullBundleChunk_PullFromBaseRevisionUntilFinishedOnTwoBranchRepo_AssembledBundleIsValid()
    {
        ...
        var (assembled, _) = Protocol.PullEntireBundle(Api, "sample2branch-hg-repo", new[] { hash }, tx);
    ...
    public void PullBundleChunk_PullFromTwoBaseRevisionsUntilFinishedOnTwoBranchRepo_AssembledBundleIsValid()
    {
        ...
        var (assembled, _) = Protocol.PullEntireBundle(Api, "sample2branch-hg-repo", hashes, tx);
    ...
    public void PullBundleChunk_2BranchRepoNoChanges_ReturnsNoChange()
    {
        ...
        var r = Api.PullBundleChunk("sample2branch-hg-repo", hashes, 0, 500, tx);
        Assert.Equal("NOCHANGE", r.Status);
    }
    ...
    public void PullBundleChunk_BaseHashIsZero_ReturnsEntireRepoAsBundle()
    {
        ...
        Api.FinishPullBundle(tx);
        var (assembled, _) = Protocol.PullEntireBundle(Api, "sample-hg-repo2", new[] { "0" }, tx);
    ...
    public void PullBundleChunk_EmptyRepositoryReturnsNoChanges()
    {
        ...
        Api.FinishPullBundle(tx);
        var r = Api.PullBundleChunk("empty-hg-repo", new[] { "0" }, 0, 50, tx);
        Assert.Equal("NOCHANGE", r.Status);
    }
    ...
    public async Task PullBundleChunk_EmptyRepoWithNonZeroBaseHash_FailsWithoutHanging()
    {
        ...
        var call = Task.Run(() => Api.PullBundleChunk("empty-hg-repo", new[] { "fakehash" }, 0, 50, tx));
    ...
    {
        // many-revs-hg-repo has 205 revisions, more than IsValidBase's page size (q = 200). A baseHash that
        ...
        _fx.SeedRepo("many-revs-hg-repo.zip");
        ...
        var call = Task.Run(() => Api.PullBundleChunk("many-revs-hg-repo", new[] { "ffffffffffff" }, 0, 50, tx));
        var finished = await Task.WhenAny(call, Task.Delay(TimeSpan.FromSeconds(30)));
        Assert.True(finished == call,
    ...
    public void PullBundleChunk_LongMakeBundle_InProgressCode()
    {
        ...
        Api.FinishPullBundle(tx);
        var r = Api.PullBundleChunk("sample-large-bundle-hg-repo", new[] { "0" }, 0, 10000000, tx);
    ...
    public void PullBundleChunk_PullUntilFinishedThenRepoChanges_ResetReceivedFromFinishPullBundle()
    {
        ...
        while (offset < bundleSize)
        {
            if (ctr == 3)
            {
                _fx.AddAndCommit("sample-hg-repo2", "fileToAdd.txt", "sample data to add");
            }
            var r = Protocol.PullFirstChunk(Api, "sample-hg-repo2", new[] { hash }, offset, chunkSize, tx);
            Assert.Equal("SUCCESS", r.Status);
